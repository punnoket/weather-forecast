...
class CitySearchFragment : BaseFragment(R.layout.fragment_city_search) {
    ...
    private fun initView() {
        with(binding) {
            citySearchViewModel.citySearchViewState
            ...
            subscriptions += buttonSearch.clicks()
                .subscribeBy(
                    onError = {},
                    onNext = {
                        citySearchViewModel.searchWeatherByName(editText.text.toString())
                    })
            ...
            toggleButtonGroup.addOnButtonCheckedListener { _, checkedId, isChecked ->
                if (isChecked) {
                    when (checkedId) {
                        R.id.buttonCelsius -> {
                            citySearchViewModel.searchWeatherByUnit(UnitEnum.CELSIUS)
                        }
                        R.id.buttonFahrenheit -> {
                            citySearchViewModel.searchWeatherByUnit(UnitEnum.FAHRENHEIT)
                        }
                    }
                }
            }
    ...
    private fun setContent(citySearchViewState: CitySearchViewState) {
        with(binding) {
            citySearchViewState.weatherResponse?.apply {
                ...
                textViewTemperature.text = getString(citySearchViewState.unit.title, main.temp.toInt())
