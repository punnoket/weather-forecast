...
class WeatherServiceTest {
    ...
    @Test
    fun `getWeather when success`() {
        // Arrange
        enqueueMockResponse("loadWeather_whenSuccess.json", mockWebServer)
        val query = mapOf("q" to "London", "units" to "metric")

        // Act & Assert
        weatherService.getWeather(query).test()
            .assertComplete()
            .assertNoErrors()
            .assertValue { result ->
                result.name == "London"
        ...
        MatcherAssert.assertThat(
            request.path,
            CoreMatchers.`is`("/weather?q=London&units=metric&appid=7b0ec359fa40d6cfb43ae8f5c0e7d141")
        )
    }
